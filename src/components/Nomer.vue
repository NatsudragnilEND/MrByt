<template>
  <form class="forrm">
    ...
    />
    <button @click.prevent="opentrorf" class="subb">Заказать</button>
  </form>
  <div class="nit" v-show="nit">Неверный номер</div>
</template>
...
const opentrorf = () => {
  if (inputValue.value.length > 13) {
    trorf.inp = inputValue.value;
    inputValue.value = trorf.inp;
    trorf.open = true;
    document.querySelector("body").classList.add("dwe");
    return (trorf.yes = true);
  } else {
    nit.value = true;
    setTimeout(() => {
      nit.value = false;
